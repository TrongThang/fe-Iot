...
export default function DeviceGrid({
    ...
    return (
        <TooltipProvider>
            ...
            >
                {devices.map((device) => {
                    const deviceCapabilities = getDeviceCapabilities(device);
                    const deviceTypeDisplayName = getDeviceTypeDisplayName(device.type, device.device_type_name, device.device_type_parent_name);
                            ...
                            {isCompact ? (
                                <>
                                    <div className="flex items-center space-x-3 flex-1">
                                        <div
                                            className={`w-12 h-12 bg-gradient-to-br ${getDeviceColor(device.type)} rounded-lg flex items-center justify-center shadow-sm text-white`}
                                        >
                                            {getDeviceIcon(device.type)}
                                        </div>
                                        <div className="flex-1">
                                            <h3 className="font-medium text-slate-900">{device.name}</h3>
                                            <p className="text-sm text-slate-500">{deviceTypeDisplayName}</p>
                                        </div>
                                    </div>
                                ...
                                <>
                                    ...
                                    {deviceCapabilities.length > 0 && (
                                        <div className="mb-3">
                                            <div className="flex flex-wrap gap-1">
                                                {deviceCapabilities.slice(0, 3).map((capability, index) => (
                                                    <Badge key={index} variant="secondary" className="text-xs px-2 py-0.5 bg-blue-50 text-blue-600 border-blue-200">
                                                        {capability}
                                                    </Badge>
                                                ))}
                                                {deviceCapabilities.length > 3 && (
                                                    <Tooltip>
                                                        <TooltipTrigger>
                                                            <Badge variant="secondary" className="text-xs px-2 py-0.5 bg-slate-50 text-slate-600 border-slate-200">
                                                                +{deviceCapabilities.length - 3} khác
                                                            </Badge>
                                                        </TooltipTrigger>
                                                        <TooltipContent>
                                                            <div className="max-w-xs">
                                                                <p className="font-medium mb-1">Tất cả chức năng:</p>
                                                                <div className="flex flex-wrap gap-1">
                                    ...
                                    <div className="flex justify-between items-center">
                                        ...
                                        <div className="flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            ...
                                            {/* Hiển thị nút unlink cho shared devices */}
                                            {device.ownership === "shared" && (
                                                <Button
                                                    variant="ghost"
                                                    size="icon"
                                                ...
                                                >
                                                    <Unlink size={14} className="text-slate-600 hover:text-orange-600" />
                                                </Button>
                                            )}
