...
export default function DeviceManagement({
	...
	// const [devices, setDevices] = useState([
	//   { 
	// id: 1, name: "Camera cổng chính",
	// room: spaceName, 
	// type: "camera", 
	// isOn: true, 
	// resolution: "1080p", 
	// lastActivity: "1 phút trước", 
	// status: "active", group: 1, 
	// group_name: "Nhóm 1", 
	// house: 1, 
	// house_name: "Nhà 1" },
	...
	const handleToggle = (deviceId) =>
		setDevices(
			devices.map((device) =>
				device.id === deviceId ? { ...device, power_status: !device.power_status, status: !device.power_status ? "active" : "inactive" } : device,
			),
		)
	...
	const getDeviceStatusColor = (status, power_status) =>
		!power_status ? "bg-gray-200 text-gray-700" : { active: "bg-green-100 text-green-700 border-green-200", warning: "bg-yellow-100 text-yellow-700 border-yellow-200", error: "bg-red-100 text-red-700 border-red-200" }[status] || "bg-gray-100 text-gray-700 border-gray-200"
	...
	const filteredDevices = devices.filter((device) => {
		...
		const matchesStatus = filterOptions.status === "all" || (filterOptions.status === "active" && device?.power_status) || (filterOptions.status === "inactive" && !device.power_status)
