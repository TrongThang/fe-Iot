...
export default function HouseTab({ houses, setHouses, isAddHousePopupOpen, setIsAddHousePopupOpen, roleUserCurrent, activeTab }) {
  const { id } = useParams()
  ...
  const [isLoading, setIsLoading] = useState(true) // Đặt isLoading thành true ban đầu
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 401) {
          ...
          throw new Error(`Lỗi khi lấy danh sách nhà: ${res.status} ${res.statusText}`)
        }
        return
      }

      const dataHouse = await res.json() // Phân tích dữ liệu JSON
      const normalizedHouses = Array.isArray(dataHouse)
        ? await Promise.all(
            dataHouse.map(async (house) => {
              const spaces = await fetchSpaces(house.house_id)
              return {
    ...
    if (confirmed) {
      try {
        ...
        if (res.ok) {
          ...
          throw new Error(errorData.message || "Xóa nhà thất bại!")
        }
      } catch (error) {
  ...
  return (
    <div className="space-y-6">
      {showSpaceList && selectedHouse ? (
        <SpaceTab houseId={selectedHouse.id} houseName={selectedHouse.name} onBack={handleBackToHouses} roleUserCurrent={roleUserCurrent} />
      ) : (
        <>
          {isLoading ? (
            <div className="text-center py-12">Đang tải danh sách nhà...</div>
          ) : (
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle className="flex items-center space-x-2">
                    ...
                    <Badge variant="secondary" className="bg-blue-100 text-blue-700">{houses.length}</Badge>
                  </CardTitle>
                  {(roleUserCurrent === "owner" || roleUserCurrent === "vice") && (
                    <Button onClick={handleAddHouse} className="bg-blue-500 hover:bg-blue-600" disabled={isAddingHouse}>
                      {isAddingHouse ? "Đang thêm..." : <>
